...
cl_va_pt_samps = normrnd(p.kcl_VA, p.kcl_VA*0.5, [N,1]);

Vr_pt_samps(Vr_pt_samps < 0.1*p.Vol_r)       = 0.1*p.Vol_r;
cl_a_pt_samps(cl_a_pt_samps < 0.1*p.kcl_A)   = 0.1*p.kcl_A;
cl_va_pt_samps(cl_va_pt_samps < 0.1*p.kcl_VA)= 0.1*p.kcl_VA;
...
for i = 1:N
    plot(T_all{i}/(60*24),Y_all{i}(:,n.VEGFAb_t)*10^-3,'linewidth',2, 'DisplayName', 'Blood')

end

title( 'VEGF-antibody Complex')
ylabel('[Ab-V] (nM)')
xlabel('time (days)')
